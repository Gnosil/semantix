...
func TestCapturePanicWritesBoundedSanitizedReport(t *testing.T) {
	...
	stack := "goroutine 7 [running]:\n" +
		"semantix/harness/internal/agent.run(" + secret + ")\n" +
	...
	if !strings.Contains(report.Stack, "semantix/harness/internal/agent.run(...)") || !strings.Contains(report.Stack, "<path>/run.go:42") {
		t.Fatalf("sanitized stack = %q", report.Stack)
	}
	if report.TopFrame != "semantix/harness/internal/agent.run <path>/run.go:42" {
