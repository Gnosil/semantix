...
2. **主题画廊** — 我的主题 / 基础配色两个分类；点击卡片仅选中；详情区隔离预览；
...
| `activeThemeId` | 仅用户或插件主题 | `desktop-theme-state.json` |
...
- 6 个基础配色 id 均为**保留名**：保存、导入、覆盖复制与删除都会拒绝冲突。
- 对基础主题执行「复制」会生成可编辑的用户主题，之后可编辑或导出。
