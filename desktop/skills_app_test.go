...
func TestSkillRootsViewCountsProjectSkills(t *testing.T) {
	...
	project := t.TempDir()
	root := filepath.Join(project, ".semantix", "skills")
...
func TestSkillRootsViewUsesActiveWorkspaceForRelativeProjectPaths(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "semantix.toml"), []byte("[skills]\npaths = [\"relative-skills\"]\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestSkillSettingsEditProjectOwnedFields(t *testing.T) {
	...
	projectConfig := filepath.Join(project, "semantix.toml")
...
func TestSkillRootsViewMarksEnvConfiguredCustomRoot(t *testing.T) {
	...
	t.Setenv("SEMANTIX_TEST_SKILL_ROOT", root)
	cfgPath := config.UserConfigPath()
	if err := os.MkdirAll(filepath.Dir(cfgPath), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(cfgPath, []byte("[skills]\npaths = [\"${SEMANTIX_TEST_SKILL_ROOT}\"]\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	wd, err := os.Getwd()
	if err != nil {
...
func TestSkillRootsViewDedupesConfiguredConventionRoot(t *testing.T) {
	...
	t.Setenv("AppData", filepath.Join(home, "AppData"))
	project := t.TempDir()
	root := filepath.Join(config.SemantixHomeDir(), "skills")
	if err := os.MkdirAll(root, 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestSkillRootsViewDedupesConfiguredProjectConventionRoot(t *testing.T) {
	...
	root = filepath.Join(cwd, ".semantix", "skills")
	cfgPath := config.UserConfigPath()
	if err := os.MkdirAll(filepath.Dir(cfgPath), 0o755); err != nil {
		t.Fatal(err)
	}
