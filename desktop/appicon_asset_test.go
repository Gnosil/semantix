...
func TestAppIconPNGUsesDarkFullCanvasRoundedBackground(t *testing.T) {
...
func TestWindowsICOUsesDarkFullCanvasRoundedBackground(t *testing.T) {
...
func assertFullCanvasRoundedIcon(t *testing.T, img image.Image, size int) {
	...
	for _, point := range edgePoints {
		...
		assertPlateDark(t, point.name, img.At(point.x, point.y))
	}
}

func assertPlateDark(t *testing.T, name string, colorValue color.Color) {
	t.Helper()

	r16, g16, b16, _ := colorValue.RGBA()
	r, g, b := uint8(r16>>8), uint8(g16>>8), uint8(b16>>8)
	if !near(r, 0x08, 2) || !near(g, 0x09, 2) || !near(b, 0x0a, 2) {
		t.Fatalf("%s edge must use the dark brand plate, got #%02x%02x%02x", name, r, g, b)
	}
}
