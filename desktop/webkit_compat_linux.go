...
// Package main provides the Wails desktop shell around the Semantix kernel.
//
...
static void semantix_fix_signal(int signum)
{
	struct sigaction st;
...
fix_signal_error:
	fprintf(stderr, "semantix: error fixing handler for signal %d: %s\n",
		signum, strerror(errno));
}

static void semantix_install_signal_handlers(void)
{
#if defined(SIGCHLD)
	semantix_fix_signal(SIGCHLD);
#endif
#if defined(SIGHUP)
	semantix_fix_signal(SIGHUP);
#endif
#if defined(SIGINT)
	semantix_fix_signal(SIGINT);
#endif
#if defined(SIGQUIT)
	semantix_fix_signal(SIGQUIT);
#endif
#if defined(SIGABRT)
	semantix_fix_signal(SIGABRT);
#endif
#if defined(SIGFPE)
	semantix_fix_signal(SIGFPE);
#endif
#if defined(SIGTERM)
	semantix_fix_signal(SIGTERM);
#endif
#if defined(SIGBUS)
	semantix_fix_signal(SIGBUS);
#endif
#if defined(SIGSEGV)
	semantix_fix_signal(SIGSEGV);
#endif
	// Do not modify SIGUSR1. JavaScriptCore owns it for conservative GC
	// stack scanning after installing its handler.
#if defined(SIGXCPU)
	semantix_fix_signal(SIGXCPU);
#endif
#if defined(SIGXFSZ)
	semantix_fix_signal(SIGXFSZ);
#endif
}

static gboolean semantix_install_signal_handlers_timeout(gpointer data)
{
	semantix_install_signal_handlers();
	int *remaining = (int *)data;
	(*remaining)--;
	return *remaining > 0 ? G_SOURCE_CONTINUE : G_SOURCE_REMOVE;
}

static void semantix_schedule_signal_handler_fix(void)
{
	...
	*remaining = 100;
	g_timeout_add_full(
		G_PRIORITY_DEFAULT,
		50,
		semantix_install_signal_handlers_timeout,
		remaining,
		g_free
	);
}
...
func scheduleWebKitSignalHandlerRepair() {
	C.semantix_schedule_signal_handler_fix()
}
...
func repairWebKitSignalHandlers() {
	C.semantix_install_signal_handlers()
}
