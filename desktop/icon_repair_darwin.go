...
func repairMacDesktopAliases(desktopDir, currentApp string) error {
	...
	for _, entry := range entries {
		...
		allowBroken := semantixExactAliasName(entry.Name())
...
func semantixExactAliasName(name string) bool {
	name = strings.TrimSpace(name)
	return strings.EqualFold(name, "Semantix") || strings.EqualFold(name, "Semantix.app")
}
...
func repairMacAlias(aliasPath, currentApp string, allowBroken bool) (bool, error) {
	...
	result := C.semantix_repair_alias(aliasCString, appCString, allow, &nativeErr)
	if nativeErr != nil {
...
func writeMacAlias(targetPath, aliasPath string) error {
	...
	defer C.free(unsafe.Pointer(aliasCString))
	var nativeErr *C.char
	result := C.semantix_write_alias(targetCString, aliasCString, &nativeErr)
	if nativeErr != nil {
		defer C.free(unsafe.Pointer(nativeErr))
	}
	if result != 0 {
...
func resolveMacAlias(aliasPath string) (string, error) {
	...
	result := C.semantix_resolve_alias(aliasCString, &target, &nativeErr)
	if target != nil {
...
func macURLReferencesSemantixBundle(rawURL string) (bool, error) {
	urlCString := C.CString(rawURL)
	defer C.free(unsafe.Pointer(urlCString))
	var nativeErr *C.char
	result := C.semantix_is_semantix_bundle_url_string(urlCString, &nativeErr)
	if nativeErr != nil {
		defer C.free(unsafe.Pointer(nativeErr))
	}
	if result < 0 {
