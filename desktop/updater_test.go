...
func TestUpdateSiblingNamesCoverEveryReplacedEntryPoint(t *testing.T) {
	...
	for _, want := range []string{"semantix-guard.exe", "semantix-launcher.exe", "semantix-update-helper.exe", "semantix-cli.exe", "Semantix.exe"} {
		if !strings.Contains(windows, want) {
			t.Errorf("Windows release unit omits %q: %q", want, windows)
		}
	}
	if strings.Contains(windows, "semantix.exe") {
		t.Fatalf("Windows release unit reintroduces the case-only CLI/launcher collision: %q", windows)
	}
	if got := updateSiblingNames("linux"); len(got) != 2 || got[0] != "semantix-guard" || got[1] != "semantix" {
...
func TestLegacyChannelsSelectOfficialPointers(t *testing.T) {
	...
	if got := downloadPage("preview"); got != "https://semantix.ensureok.ai/?download=desktop#start" {
		t.Errorf("legacy preview download page = %q", got)
	}
	if got := manifestDownloadPage("preview", "https://semantix.ensureok.ai/?channel=preview&download=desktop#start"); got != "https://semantix.ensureok.ai/?download=desktop#start" {
	...
	for _, unsafe := range []string{
		"javascript:alert(1)",
		"http://semantix.ensureok.ai/#start",
		"https://user@semantix.ensureok.ai/#start",
	} {
...
func TestDesktopManifestValidation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mutate: func(m *update.Manifest) {
				...
				asset.URL = strings.Replace(asset.URL, "Semantix-", "Other-", 1)
		...
		},
		{
			...
			mutate: func(m *update.Manifest) {
				asset := m.Platforms["darwin-arm64"]
				asset.URL = strings.Replace(asset.URL, "dl.semantix.ensureok.ai", "example.com", 1)
				asset.Sig = asset.URL + ".minisig"
				m.Platforms["darwin-arm64"] = asset
			},
		},
		...
		},
		{
			...
			mutate: func(m *update.Manifest) {
				m.DownloadPage = "https://semantix.ensureok.ai/?channel=stable&download=desktop#start"
			},
		},
	}
	...
	t.Run("unified GitHub release base", func(t *testing.T) {
		...
		newBase := "https://github.com/Gnosil/semantix/releases/download/v1.19.0/"
		for key, asset := range manifest.Platforms {
	...
	t.Run("mixed official bases", func(t *testing.T) {
		...
		asset.URL = strings.Replace(
			...
			"https://github.com/Gnosil/semantix/releases/download/desktop-v1.18.0/",
			1,
		)
		asset.Sig = asset.URL + ".minisig"
...
func TestValidateUpdateRedirect(t *testing.T) {
	tests := []struct {
		...
		{name: "Semantix first-party redirect", target: "https://dl.semantix.ensureok.ai/file"},
		...
		{name: "Semantix suffix spoof", target: "https://dl.semantix.ensureok.ai.evil.invalid/file", wantError: true},
		{name: "GitHub suffix spoof", target: "https://release-assets.githubusercontent.com.evil.invalid/file", wantError: true},
		{name: "explicit port", target: "https://dl.semantix.ensureok.ai:443/file", wantError: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestSaveCachedUpdateMarksEvaluateDownloaded(t *testing.T) {
	...
	asset := update.Asset{
		URL:    "https://dl.semantix.ensureok.ai/desktop-v9.9.9/Semantix-linux-amd64.tar.gz",
		Size:   int64(len(data)),
...
func TestDebCacheRequiresSignatureAndRejectsTarballReuse(t *testing.T) {
	...
	data := []byte("deb-bytes")
	asset := update.Asset{
		URL:    "https://dl.semantix.ensureok.ai/desktop-v9.9.9/Semantix-linux-amd64.deb",
		Size:   int64(len(data)),
		SHA256: sha256Hex(data),
	}
...
func TestExtractBinary(t *testing.T) {
	...
	tw := tar.NewWriter(gz)
	files := map[string][]byte{"README": []byte("ignore me"), "semantix-desktop": want}
	...
	got, err := extractBinary(buf.Bytes(), "semantix-desktop")
	if err != nil {
...
func TestExtractLinuxReleaseUnitRejectsAmbiguousMembers(t *testing.T) {
	...
	headers := []tar.Header{
		base("semantix-desktop", []byte("desktop")),
		base("semantix-guard", []byte("guard")),
		base("semantix", []byte("cli")),
	}
	bodies := [][]byte{[]byte("desktop"), []byte("guard"), []byte("cli")}
	if got, err := extractLinuxReleaseUnit(makeArchive(t, headers, bodies)); err != nil ||
		string(got["semantix-desktop"]) != "desktop" {
		t.Fatalf("complete release extraction = %v, %q", err, got["semantix-desktop"])
	}

	duplicateHeaders := append(append([]tar.Header(nil), headers...), base("nested/semantix", []byte("duplicate")))
	...
	nonRegular[1] = tar.Header{Name: "semantix-guard", Typeflag: tar.TypeSymlink, Linkname: "outside"}
...
func TestApplyLinuxVersionedActivatesWithoutPersistingGuard(t *testing.T) {
	...
	tw := tar.NewWriter(gz)
	for _, name := range []string{"semantix-desktop", "semantix-guard", "semantix"} {
		body := []byte("new-" + name)
		if err := tw.WriteHeader(&tar.Header{Name: name, Mode: 0o755, Size: int64(len(body)), Typeflag: tar.TypeReg}); err != nil {
			t.Fatal(err)
		}
	...
	if err != nil || string(data) != "new-semantix-desktop" {
		t.Fatalf("active desktop=%q err=%v", data, err)
	}
	for _, guardPath := range []string{
		filepath.Join(root, "semantix-guard"),
		filepath.Join(root, "versions", "v1.20.1", "semantix-guard"),
	} {
...
func TestApplyLinuxHoldsReleaseUnitLockDuringReplace(t *testing.T) {
	...
	exe := filepath.Join(dir, "semantix-desktop")
...
func TestDownloadRejectsAssetSizeAboveMaximum(t *testing.T) {
	if _, err := download(
		...
		&http.Client{},
		nil,
		"https://dl.semantix.ensureok.ai/file",
