...
func TestWindowsUpdateHandoffArgsCarryParentInstallAndRelaunch(t *testing.T) {
	got := windowsUpdateHandoffArgs(
		4242,
		`C:\Users\Jane Doe\AppData\Local\Semantix\updates\Semantix-windows-amd64-installer.exe`,
		strings.Repeat("a", 64),
		`D:\Tools\Semantix App`,
		`D:\Tools\Semantix App\semantix-desktop.exe`,
		"v1.6.0",
	...
	want := []string{
		"--parent-pid", "4242",
		"--installer", `C:\Users\Jane Doe\AppData\Local\Semantix\updates\Semantix-windows-amd64-installer.exe`,
		"--installer-sha256", strings.Repeat("a", 64),
		"--to-version", "v1.6.0",
		"--created-at", "2026-07-29T00:00:00Z",
		"--transaction-id", "transaction-1",
		"--install-dir", `D:\Tools\Semantix App`,
		"--relaunch", `D:\Tools\Semantix App\semantix-desktop.exe`,
	}
...
func TestWindowsVersionedUpdateHandoffArgsDoNotRequireLegacyPendingIdentity(t *testing.T) {
	got := windowsVersionedUpdateHandoffArgs(
		4242,
		`C:\Temp\Semantix-installer.exe`,
		strings.Repeat("b", 64),
		`D:\Tools\Semantix`,
		`D:\Tools\Semantix\semantix-launcher.exe`,
		"v1.20.0",
	)
	want := []string{
		"--parent-pid", "4242",
		"--installer", `C:\Temp\Semantix-installer.exe`,
		"--installer-sha256", strings.Repeat("b", 64),
		"--to-version", "v1.20.0",
		"--install-layout", "versioned-v1",
		"--install-dir", `D:\Tools\Semantix`,
		"--relaunch", `D:\Tools\Semantix\semantix-launcher.exe`,
	}
	if strings.Join(got, "\x00") != strings.Join(want, "\x00") {
		t.Fatalf("args = %#v, want %#v", got, want)
	}
...
func TestWindowsInstallerScriptWaitsBeforeCopyingExecutable(t *testing.T) {
	...
	for _, want := range []string{
		`!define SEMANTIX_LEGACY_UNINST_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\Semantix"`,
		`!define SEMANTIX_LEGACY_PRODUCT_KEY "Software\semantix\Semantix"`,
	...
	finishPageHook := strings.Index(script, "!define MUI_PAGE_CUSTOMFUNCTION_PRE semantix.skipFinishPageForUpdate")
	...
	wait := strings.Index(script, "Call semantix.waitForExecutableUnlock")
	...
	stageBranch := strings.Index(script, "StrCmp $SemantixStageMode \"1\" semantix_stage_payload")
	...
	if !strings.Contains(script, "Goto semantix_section_done") {
	...
	writeCurrent := strings.Index(script, `!insertmacro semantix.writeUninstaller`)
	deleteLegacy := strings.Index(script, `!insertmacro semantix.deleteLegacyInstallerStateIfOwned`)
	...
	legacyMacro := script[strings.Index(script, `!macro semantix.deleteLegacyInstallerStateIfOwned`):strings.Index(script, `!macro semantix.deleteUninstaller`)]
	deleteLegacyLocation := strings.Index(legacyMacro, `DeleteRegValue HKCU "${SEMANTIX_LEGACY_PRODUCT_KEY}" ""`)
	deleteLegacyAlias := strings.Index(legacyMacro, `DeleteRegKey HKCU "${SEMANTIX_LEGACY_UNINST_KEY}"`)
	...
	metadataBranch := strings.Index(script, `semantix_stage_payload:`)
	metadataFile := strings.Index(script, `File "/oname=${SEMANTIX_PAYLOAD_MANIFEST}"`)
	normalInstall := strings.Index(script, `semantix_normal_install:`)
...
func TestWindowsInstallerUsesPreviousDirectoryAsManualInstallDefault(t *testing.T) {
	...
	for _, want := range []string{
		...
		`InstallDir "${SEMANTIX_DEFAULT_INSTALLDIR}"`,
	...
	pageHook := strings.Index(script, "!define MUI_PAGE_CUSTOMFUNCTION_PRE semantix.skipSetupPageForUpdate\n!insertmacro MUI_PAGE_DIRECTORY")
	if page < 0 || pageHook < 0 || pageHook > page {
		t.Fatal("directory selection page must remain available for manual installs")
	}
	if strings.Contains(script, `StrCpy $SemantixUpdateMode "1"
	Goto semantix_show_setup_page`) {
...
func TestDesktopBuildScriptCompilesAndPackagesWindowsUpdateHelper(t *testing.T) {
	...
	script := string(data)
	for _, want := range []string{
		`UPDATE_HELPER="semantix-update-helper.exe"`,
