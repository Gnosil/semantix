...
func TestDiagnoseBotConnectionBuildsReportDetailForMissingSecret(t *testing.T) {
	...
	if _, err := app.upsertBotConnection(config.BotConnectionConfig{
		...
		Enabled:       true,
		Status:        "connected",
		WorkspaceRoot: "/Users/alice/work/semantix",
...
func TestRememberBotConnectionRemoteStoresStableScope(t *testing.T) {
	...
	if _, err := app.upsertBotConnection(config.BotConnectionConfig{
		...
		Enabled:       true,
		Status:        "connected",
		WorkspaceRoot: "/tmp/semantix-project",
	}, nil); err != nil {
	...
	if projectMapping.Scope != "project" || projectMapping.WorkspaceRoot != "/tmp/semantix-project" || projectMapping.RemoteID != "wxid_project" {
