...
// by substring (file names embed the key, e.g. Semantix-darwin-arm64.zip), so the
...
// (semantix.key) and the public key (semantix.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "semantix.key")
	pubPath := filepath.Join(dir, "semantix.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "Semantix desktop release")
		out := f + ".minisig"
...
func genManifest(dir, version, tag string, notesVersions ...string) error {
	...
	m := update.Manifest{
		Version:         version,
		DownloadPage:    "https://semantix.ensureok.ai/?download=desktop#start",
		ReleaseNotesURL: "https://semantix.ensureok.ai/changelog/" + notesVersion + "/",
