...
func TestIsDpkgOwnedSemantixRejectsEmpty(t *testing.T) {
	if isDpkgOwnedSemantix("") {
		t.Fatal("empty path must not be dpkg-owned")
	}
	if isDpkgOwnedSemantix("relative/path") {
		t.Fatal("relative path must not be dpkg-owned")
	}
	// A random absolute path that is not packaged.
	if isDpkgOwnedSemantix(filepath.Join(t.TempDir(), "semantix-desktop")) {
