...
func TestWindowsWebView2SmokeUsesExternalProductionBinaryContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		`$env:SEMANTIX_HOME = $smokeHome`,
		`$env:SEMANTIX_STATE_HOME = $smokeState`,
		`$env:SEMANTIX_CACHE_HOME = $smokeCache`,
		`close_behavior = "quit"`,
	...
	for _, forbidden := range []string{
		...
		"Semantix lost its main window or WebView2 renderer during the health window",
	} {
...
func TestWindowsReleaseSignsPayloadBeforeRepackaging(t *testing.T) {
	...
	for _, want := range []string{
		...
		`github.repository == 'Gnosil/semantix'`,
		...
		`go run ./cmd/sign sign ../signed-payload/semantix-payload.json`,
		`go run ./cmd/sign verify ../signed-payload/semantix-payload.json`,
		`SEMANTIX_REQUIRE_PAYLOAD_MANIFEST: "1"`,
	...
	for _, want := range []string{
		...
		`SEMANTIX_REQUIRE_PAYLOAD_MANIFEST`,
		`"-DARG_SEMANTIX_SIGNED_UNINSTALLER=${uninstaller_path}"`,
	...
	for _, want := range []string{
		...
		`Portable = (Join-Path $activeDir "semantix-desktop.exe")`,
		`Portable = "semantix-desktop.exe"`,
...
func TestWindowsPackagerRejectsMissingOrPartialRequiredPayloadManifest(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.manifest {
				if err := os.WriteFile(filepath.Join(payload, "semantix-payload.json"), []byte("{}"), 0o600); err != nil {
					t.Fatal(err)
				}
			}
			if tc.signature {
				if err := os.WriteFile(filepath.Join(payload, "semantix-payload.json.minisig"), []byte("sig"), 0o600); err != nil {
					t.Fatal(err)
				}
			}
			cmd := exec.Command("bash", "../scripts/package-windows-desktop.sh", "amd64", payload)
			cmd.Env = append(os.Environ(), "SEMANTIX_REQUIRE_PAYLOAD_MANIFEST=1")
...
func TestSignPathConfigurationsCoverExactWindowsPayload(t *testing.T) {
	expected := map[string]bool{
		"semantix-desktop.exe":       true,
		"semantix-guard.exe":         true,
		"semantix-launcher.exe":      true,
		"semantix-update-helper.exe": true,
		"semantix-cli.exe":           true,
		"semantix-uninstall.exe":     true,
	}
