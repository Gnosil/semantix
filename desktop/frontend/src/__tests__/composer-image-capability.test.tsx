...

{
  ...
  installBridgeApp({
    SavePastedImage: async () => {
      saveCalls += 1;
      return ".semantix/attachments/mock.png";
    },
...

{
  ...
  installBridgeApp({
    SavePastedImage: async () => ".semantix/attachments/mock.png",
    AttachmentDataURL: async () => "data:image/png;base64,iVBORw0KGgo=",
  });
  ...
  ok(sent[0]?.submit?.includes("@.semantix/attachments/mock.png") === true, "submitted text retains the local image attachment ref");
...

{
  ...
  const { root, paint } = renderUserMessage("check @[photo.png](.semantix/attachments/mock.png)");
  await paint();
